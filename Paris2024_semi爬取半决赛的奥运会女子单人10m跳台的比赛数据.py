...
print("=== Paris 2024 Women's 10m Platform Semifinal Data Extraction ===")
...
url = "https://www.worldaquatics.com/competitions/2943/olympic-games-paris-2024/results?disciplines=&event=9640d00d-e468-48d4-b47b-cf81ddb83f29&unit=semifinals"

print(f"🌐 Extracting data from: Paris 2024 Women's 10m Platform SEMIFINAL")
print(f"🔗 URL: {url}")
...
def create_tables(connection):
    """创建数据表 - 2024_Paris SEMIFINAL"""
    try:
        cursor = connection.cursor()

        # 创建运动员排名表 - 2024_Paris SEMIFINAL（包含Age和Olympic_Games字段）
        athlete_table_sql = """
        CREATE TABLE IF NOT EXISTS `2024_Paris_Women10m_SEMIFINAL_Athlete_Rankings` (
            ...
            Competition_Phase VARCHAR(10) DEFAULT 'SEMIFINAL',
            Olympic_Games VARCHAR(20) DEFAULT '2024_Paris',
            extraction_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )
        """

        # 创建跳水详细分数表 - 2024_Paris SEMIFINAL（包含Olympic_Games字段）
        dive_table_sql = """
        CREATE TABLE IF NOT EXISTS `2024_Paris_Women10m_SEMIFINAL_Detailed_Scores` (
            id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        connection.commit()
        print("✅ 2024_Paris SEMIFINAL Database tables created/verified successfully")
...
def save_to_database(connection, athlete_list, dive_list):
    """保存数据到数据库 - 2024_Paris SEMIFINAL"""
    try:
        cursor = connection.cursor()

        # 首先检查表是否存在，如果不存在则创建
        cursor.execute("SHOW TABLES LIKE '2024_Paris_Women10m_SEMIFINAL_Athlete_Rankings'")
        if not cursor.fetchone():
            print("⚠️ 2024_Paris SEMIFINAL tables not found, creating tables...")
            create_tables(connection)

        # 清空现有2024_Paris SEMIFINAL数据
        print("🔄 Clearing existing 2024_Paris SEMIFINAL data from tables...")
        cursor.execute(
            "DELETE FROM `2024_Paris_Women10m_SEMIFINAL_Athlete_Rankings` WHERE Olympic_Games = '2024_Paris' AND Competition_Phase = 'SEMIFINAL'")
        cursor.execute(
            "DELETE FROM `2024_Paris_Women10m_SEMIFINAL_Detailed_Scores` WHERE Olympic_Games = '2024_Paris' AND Competition_Phase = 'SEMIFINAL'")

        # 插入运动员排名数据 - 2024_Paris SEMIFINAL
        athlete_insert_sql = """
        INSERT INTO `2024_Paris_Women10m_SEMIFINAL_Athlete_Rankings` 
        (`Rank`, Athlete, Age, Country, Total_Points, Competition_Phase, Olympic_Games)
        VALUES (%s, %s, %s, %s, %s, %s, %s)
        """

        athlete_data = [(a['Rank'], a['Athlete'], a['Age'], a['Country'], a['Total_Points'], 'SEMIFINAL', '2024_Paris')
                        for a in athlete_list]

        if athlete_data:
            cursor.executemany(athlete_insert_sql, athlete_data)
            print(f"✅ Saved {len(athlete_data)} athlete records to 2024_Paris SEMIFINAL database")

        # 插入跳水详细分数数据 - 2024_Paris SEMIFINAL
        dive_insert_sql = """
        INSERT INTO `2024_Paris_Women10m_SEMIFINAL_Detailed_Scores` 
        ...
        for dive in dive_list:
            dive_tuple = (
                ...
                'SEMIFINAL', '2024_Paris'
            )
            dive_data.append(dive_tuple)

        if dive_data:
            cursor.executemany(dive_insert_sql, dive_data)
            print(f"✅ Saved {len(dive_data)} dive records to 2024_Paris SEMIFINAL database")
        ...
        cursor = connection.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM `2024_Paris_Women10m_SEMIFINAL_Athlete_Rankings` WHERE Olympic_Games = '2024_Paris' AND Competition_Phase = 'SEMIFINAL'")
        athlete_count = cursor.fetchone()[0]

        cursor.execute(
            "SELECT COUNT(*) FROM `2024_Paris_Women10m_SEMIFINAL_Detailed_Scores` WHERE Olympic_Games = '2024_Paris' AND Competition_Phase = 'SEMIFINAL'")
        dive_count = cursor.fetchone()[0]

        print(f"\n📊 2024_Paris SEMIFINAL Database Statistics:")
        print(f"  Athlete records in 2024_Paris SEMIFINAL database: {athlete_count}")
        print(f"  Dive records in 2024_Paris SEMIFINAL database: {dive_count}")
...
try:
    ...
    print("Page loaded successfully, executing JavaScript to extract 2024_Paris SEMIFINAL data...")
    ...
    if all_data:
        print(f"Successfully extracted 2024_Paris SEMIFINAL data for {len(all_data)} athletes")
        ...
        # 1. Save to CSV files - 2024_Paris SEMIFINAL
        if athlete_list:
            df_athletes = pd.DataFrame(athlete_list)
            athletes_filename = "data/2024_Paris_Women10m_SEMIFINAL_Athlete_Rankings.csv"
            df_athletes.to_csv(athletes_filename, index=False, encoding='utf-8-sig')
            print(f"✅ 2024_Paris SEMIFINAL athlete rankings saved to CSV: {athletes_filename}")
        ...
        if dive_list:
            ...
            dives_filename = "data/2024_Paris_Women10m_SEMIFINAL_Detailed_Scores.csv"
            df_dives.to_csv(dives_filename, index=False, encoding='utf-8-sig')
            print(f"✅ 2024_Paris SEMIFINAL detailed diving scores saved to CSV: {dives_filename}")
            print(f"📊 2024_Paris SEMIFINAL total {len(dive_list)} records ({len(all_data)} athletes × 5 dives each)")

        # 2. Save to database - 2024_Paris SEMIFINAL
        ...
        # Display statistics
        print(f"\n📈 2024_Paris SEMIFINAL Data Extraction Statistics:")
        print(f"  Number of athletes in 2024_Paris SEMIFINAL: {len(all_data)}")
        print(f"  Total number of dives in 2024_Paris SEMIFINAL: {len(dive_list)}")
        ...
        try:
            ...
            if valid_scores:
                print(f"  Highest total score in 2024_Paris SEMIFINAL: {max(valid_scores):.1f}")
        except:
            print(f"  Highest total score in 2024_Paris SEMIFINAL: Calculation error")

    else:
        print("No 2024_Paris SEMIFINAL data extracted")

except Exception as e:
    print(f"Error extracting 2024_Paris SEMIFINAL data: {e}")
    import traceback
    ...
    print("🎉 2024_Paris SEMIFINAL data extraction and storage completed!")