...
class SingleCompetitionAnalyzer:
    ...
    def create_visualizations(self):
        ...
        self._plot_stability_radar(ax3)
        ...
        plt.suptitle(f'Competition Analysis: {self.competition_name} - {self.athlete}',
    ...
    def _plot_stability_radar(self, ax):
        """绘制稳定性雷达图"""
        # 计算各个维度的稳定性指标
        ...
        # 归一化值（0-1）
        cv_norm = 1 - min(metrics['score_cv'] / 30, 1)  # CV越小越好

        # 开场稳定性（基于前2跳平均分）
        opening_norm = min(temporal_metrics['opening_stability'] / 100, 1)

        # 中期稳定性（基于关键跳）
        middle_norm = min(temporal_metrics['key_dive_performance'] / 100, 1) if temporal_metrics[
                                                                                    'key_dive_performance'] > 0 else 0.5

        # 收尾稳定性
        closing_norm = min(temporal_metrics['closing_stability'] / 100, 1)

        # 难度适应性（基于调整后的CV）
        adj_cv = metrics.get('adjusted_cv', 0)
        diff_adapt_norm = 1 - min(adj_cv / 30, 1) if adj_cv > 0 else 0.5

        values = [cv_norm, opening_norm, middle_norm, closing_norm, diff_adapt_norm]
        ...
        ax.set_ylim(0, 1)
        ax.set_title('Stability Radar Chart', y=1.1, fontsize=12)
...
def analyze_2023_world_championships(df):
    """
    专门分析2023年世界锦标赛
    """
    print("=" * 80)
    print("ANALYSIS OF 2023 WORLD CHAMPIONSHIPS - CHEN YUXI")
    print("=" * 80)

    # 筛选2023年世界锦标赛数据
    competition_mask = df['Event'] == '2023_world_championships'
    competition_data = df[competition_mask].copy()

    if len(competition_data) == 0:
        print("Error: No data found for 2023_world_championships")
        return None

    print(f"Found {len(competition_data)} dives for 2023 World Championships")
    print("\nDive Details:")
    print("-" * 60)
    print(competition_data[['Dive_Number', 'Dive_Code', 'Difficulty_Degree', 'Dive_Points']].to_string(index=False))
    ...
    # 保存图表
    output_filename = "2023_world_championships_chen_yuxi_analysis.png"
    ...
    if trend == 'Declining':
        ...
        recommendations.append("Excellent competitive adaptation - maintain this pattern")
    ...
    if not recommendations:
        recommendations.append("Excellent overall performance - maintain current training regimen")
...
if __name__ == "__main__":
    ...
    try:
        if not os.path.exists(file_path):
            ...
            # 专门分析2023年世界锦标赛
            results = analyze_2023_world_championships(df)

            if results:
                print("\nAnalysis completed successfully!")
                print(f"Check the generated visualization: 2023_world_championships_chen_yuxi_analysis.png")
